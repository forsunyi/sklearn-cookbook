...
# 如果两个向量相似度高说明距离短，那么1/(1+距离)应该趋向于1
# 如果两个向量相似度低说明距离长，那么1/(1+距离)应该趋向于0
def ecludSim(inA, inB):
    return 1.0 / (1.0 + la.norm(inA - inB))


# 皮尔逊相关系数的计算方法corrcoef()，参数rowvar=0表示对列求相似度，
# 这里的0.5+0.5*corrcoef()是为了将范围归一化放到0和1之间
# 皮尔逊相关系数实际就是inA和inB的协方差除以各自标准差的乘积
...
def cosSim(inA, inB):
    num = float(inA.T * inB) # 两个向量inA和inB的点积
    denom = la.norm(inA) * la.norm(inB) # 两个向量inA和inB的各自模的乘积
...
def recommend(dataMat, user, simMeas=cosSim, estMethod=svdEst, percentage=0.9):
    ...
    return itemScores  # 返回前N大评分值的item名，及其预测评分值


testdata = loadExData()
result = recommend(testdata, 1, percentage=0.8)
print(result)
